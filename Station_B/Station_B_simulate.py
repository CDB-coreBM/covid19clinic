...
deepwell_cross_section_area = math.pi * D_deepwell**2 / \
    4  # deepwell cilinder cross secion area
...
def run(ctx: protocol_api.ProtocolContext):
    ...
    def move_vol_multi(pipet, reagent, source, dest, vol, air_gap_vol, x_offset,
        ...
        pipet.dispense(vol + air_gap_vol + 20, dest.top(z=-1),
...
# PREMIX BEADS
    ...
    # Mixing
    custom_mix(
        m300, Beads, Beads.reagent_reservoir[Beads.col], vol=180, rounds=20, blow_out=True)
...
###############################################################################
    ...
    for i in range(num_cols):
        ...
        # Mixing
        custom_mix(m300, Elution,
                   work_destinations[i], vol=50, rounds=4, blow_out=True)
        m300.drop_tip(home_after=True)
        tip_track['counts'][m300] += 8
