...
def run(ctx: protocol_api.ProtocolContext):
    ...
    pcr_plate = tempdeck.load_labware(
        'roche2_96_wellplate_100ul', 'PCR plate')
    tempdeck.set_temperature(25)
    tuberack = ctx.load_labware(
        'bloquealuminio_24_wellplate_1500ul', '3',
    ...
    sample_dest_sets = [row[i*3:(i+1)*3] for i in range(12//3) for row in pcr_plate.rows()][:NUM_SAMPLES]
